		...
		<div class="mypoint">
			<div id="section" class="box has-text-centered-tablet container is-widescreen">
				<div action="success.html" method="post">
					...
					<h1
						style="font-size: 20px; font-family:微软雅黑; color: dodgerblue; margin-bottom: 20px; text-align: center; ">
						<b>--- 用户登录 ---</b></h1>
					...
					<div class="geshi">
						...
						<div class="field mt-2" v-if="logshow == '2'">
							<label class="label has-text-left-tablet" style="color: darkviolet; ">用户名
								...
								<div class="control">
									<input class="input is-info" type="text" name="userName" v-model="userdata.name"
										placeholder="请输入您的用户名" maxlength="20" style="border-width:2px; " pattern="\w{3,20}"
										required />
								</div>
							</label>
						...
						<div class="field mt-2" v-if="logshow == '2'">
							<label class="label content has-text-left-mobile has-text-left-tablet"
								style="color: darkviolet;">密码
								<div class="control">
									<input class="input is-info" v-model="userdata.password" type="password" name="pwd"
										placeholder="请输入您的密码" maxlength="16" style="border-width:2px; " pattern="\w{6,16}"
										required />
								</div>
							</label>
						</div>
						...
						<div class="field mt-2" v-if="logshow == '1'">
							<label class="label has-text-left-tablet" style="color: darkviolet; ">手机号
								...
								<div class="control">
									<input class="input is-info" type="text" name="userName" v-model="userphone.phone"
										placeholder="请输入您的用户名" maxlength="20" style="border-width:2px; " pattern="\w{3,20}"
										required />
								</div>
							</label>
						...
						<div class="field mt-2" v-if="logshow == '1'">
							<label class="label content has-text-left-mobile has-text-left-tablet"
								style="color: darkviolet;">验证码
								<div class="control">
									<input class="input iphone-wrap is-info" v-model="userphone.code" type="password"
										name="pwd" placeholder="请输入验证码" maxlength="16" style="border-width:2px; "
										pattern="\w{6,16}" required />
									<button class="codebtn button is-info is-outlined is-normal" :disabled="btnclicn"
										@click="getcode">{{ btntext }}</button>
								</div>
							</label>
						</div>
						...
						<div class="control" style="margin-top: 15px; text-align: center; ">
							<input type="submit" name="sub" value="点击登录" :disabled="lontnshow" @click="submint"
								style="width:100%" class="button is-info is-outlined is-normal mt-2">
						</div>

						<div class="control" style="margin-top: 15px; text-align: center; ">
							<span style="cursor: pointer;" @click="logshow = '2'">账号密码登陆</span> | <span
								style="cursor: pointer;" @click="logshow = '1'">手机号登陆</span> |
